 ...
 */
package org.eximeebpms.bpm.spring.boot.starter.configuration.condition;
...
public class NeedsHistoryAutoConfigurationCondition extends SpringBootCondition {
  ...
  protected boolean needsAdditionalConfiguration(ConditionContext context) {
    String historyLevel = context.getEnvironment().getProperty("eximeebpms.bpm.history-level");
  ...
  protected boolean isHistoryAutoSupported() {
    // FIXME see
    // https://github.com/eximeebpms/eximeebpms-bpm-spring-boot-starter/issues/30
    return false;
