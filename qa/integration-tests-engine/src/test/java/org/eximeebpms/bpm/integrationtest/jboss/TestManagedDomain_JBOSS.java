 ...
 */
package org.eximeebpms.bpm.integrationtest.jboss;
...
public class TestManagedDomain_JBOSS {
  ...
  public void shouldBeAbleToLookupDefaultProcessEngine() {
    try {
      Assert.assertNotNull(InitialContext.doLookup("java:global/eximeebpms-bpm-platform/process-engine/default"));
    } catch (NamingException e) {
      Assert.fail("Could not lookup default process engine");
    }

    try {
      Assert.assertNotNull(InitialContext.doLookup("java:global/eximeebpms-bpm-platform/process-engine/someNonExistingEngine"));
