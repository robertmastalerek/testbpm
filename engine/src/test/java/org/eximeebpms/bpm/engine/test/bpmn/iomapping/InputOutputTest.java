...
public class InputOutputTest extends PluggableProcessEngineTest {
  ...
  public void testInterruptingEventSubprocessIoSupport() {
    try {
      ...
      // happy path
      testRule.assertTextPresent("eximeebpms:inputOutput mapping unsupported for element type 'subProcess' with attribute 'triggeredByEvent = true'", e.getMessage());
  ...
  public void testMIOutputMappingDisallowed() {
    try {
      ...
      testRule.assertTextPresent("eximeebpms:outputParameter not allowed for multi-instance constructs", e.getMessage());
